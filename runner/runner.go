...
import (
	...
	customport "github.com/t101804/httpr/common/customports"
	fileutilz "github.com/t101804/httpr/common/fileutil"
	"github.com/t101804/httpr/common/httputilz"
	"github.com/t101804/httpr/common/httpx"
	"github.com/t101804/httpr/common/slice"
	"github.com/t101804/httpr/common/stringz"
)
...
func (r *Runner) prepareInput() int {
	var numHosts int
...
func (r *Runner) RunEnumerationWithID() {
	...
	go func(output chan Result, nextSteps ...chan Result) {
		...
		// initiate the ID now Proccess
		i := 0
		if len(r.options.requestURIs) > 1 {
			total = total * len(r.options.requestURIs)
		}
		for resp := range output {
			// every output add ID proccess
			i++

			if r.options.OnResult != nil {
				// Attach Result id and total proccess to Results
				resp.ID = i
				resp.Total = total
			...
			//nolint:errcheck // this method needs a small refactor to reduce complexity
			if plainFile != nil {
				plainFile.WriteString(resp.URL + "\n")
			}
...
func SaveOnlyUrl(url string, file *os.File) {
	if _, err := file.Write([]byte(url)); err != nil {
